...
SUB Throw_mANUAL_Switch(TurnOutNum,onoff,ctc_Grid)
	$Switch(ctc_Grid)=$Switch(ctc_Grid)~
	tortoise[TurnoutNum]=tortoise[TurnoutNum]~
ENDSUB
...
'**	Initialize panel locations of tortoise controls - left click for normal route, right for diverging
	...
	Turnout_Grid[8]=(7,26,4),
	Turnout_Grid[9]=(23,2,4),
...
'**	Initialize sprite shapes to overlay on assign cab pushbuttons
	...
	Train_E_Sprite[5]=Loco_north,
	Train_W_Sprite[5]=Loco_south,
...
'**			Left click turns tortoise off, and indates diverging route with red on panel			
	...
	WHEN TEST<>TESTOLD DO 			
			Throw_MANUAL_Switch(7,off,Turnout_Grid[7]),
			TESTOLD=TEST		
...
'**	LOOP through East and West InfraRed detectors to determine block occupancy
	' All commented lines below to be restored WHEN all modules are in place
	...
	WHEN IRD_East[0]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[0])= IR_Detector_Shape_E[0] in red
	WHEN IRD_West[0]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[0])= IR_Detector_Shape_W[0] in red
	WHEN IRD_East[0]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[0])
	WHEN IRD_West[0]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[0])

	WHEN IR_EOT_Detector[1]=occupied DO
		$Draw Sprite(IR_EOT_Detector_Grid[1])= IR_EOT_Detector_Shape[1] in red
	WHEN IR_EOT_Detector[1]=VACANT DO
		$Erase Sprite(IR_EOT_Detector_Grid[1])

	WHEN IRD_East[1]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[1])= IR_Detector_Shape_E[1] in red
	WHEN IRD_West[1]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[1])= IR_Detector_Shape_W[1] in red
	WHEN IRD_East[1]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[1])
	WHEN IRD_West[1]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[1])


	WHEN IRD_East[2]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[2])= IR_Detector_Shape_E[2] in red
	WHEN IRD_West[2]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[2])= IR_Detector_Shape_W[2] in red
	WHEN IRD_East[2]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[2])
	WHEN IRD_West[2]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[2])

	WHEN IRD_East[3]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[3])= IR_Detector_Shape_E[3] in red
	WHEN IRD_West[3]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[3])= IR_Detector_Shape_W[3] in red
	WHEN IRD_East[3]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[3])
	WHEN IRD_West[3]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[3])

	WHEN IRD_East[4]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[4])= IR_Detector_Shape_E[4] in red
	WHEN IRD_West[4]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[4])= IR_Detector_Shape_W[4] in red
	WHEN IRD_East[4]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[4])
	WHEN IRD_West[4]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[4])

	WHEN IRD_East[5]=occupied DO
		$Draw Sprite(IR_Detector_Grid_E[5])= IR_Detector_Shape_E[5] in red
	WHEN IRD_West[5]=occupied DO
		$Draw Sprite(IR_Detector_Grid_W[5])= IR_Detector_Shape_W[5] in red
	WHEN IRD_East[5]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_E[5])
	WHEN IRD_West[5]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[5])

'	WHEN IRD_East[6]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[6])= IR_Detector_Shape_E[6] in red
'	WHEN IRD_West[6]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[6])= IR_Detector_Shape_W[6] in red
'	WHEN IRD_East[6]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[6])
'	WHEN IRD_West[6]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_W[6])
'
'	WHEN IRD_East[7]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[7])= IR_Detector_Shape_E[7] in red
'	WHEN IRD_West[7]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[7])= IR_Detector_Shape_W[7] in red
'	WHEN IRD_East[7]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[7])
'	WHEN IRD_West[7]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_W[7])
'
'	WHEN IRD_East[8]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[8])= IR_Detector_Shape_E[8] in red
'	WHEN IRD_West[8]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[8])= IR_Detector_Shape_W[8] in red
'	WHEN IRD_East[8]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[8])
'	WHEN IRD_West[8]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_W[8])
'
'	WHEN IRD_East[9]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[9])= IR_Detector_Shape_E[9] in red
'	WHEN IRD_West[9]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[9])= IR_Detector_Shape_W[9] in red
'	WHEN IRD_East[9]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[9])
'	WHEN IRD_West[9]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_W[9])
'
'	WHEN IRD_East[10]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[10])= IR_Detector_Shape_E[10] in red
'	WHEN IRD_West[10]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[10])= IR_Detector_Shape_W[10] in red
'	WHEN IRD_East[10]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[10])
'	WHEN IRD_West[10]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_W[10])
'
'	WHEN IRD_East[11]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_E[11])= IR_Detector_Shape_E[11] in red
'	WHEN IRD_West[11]=occupied DO
'		$Draw Sprite(IR_Detector_Grid_W[11])= IR_Detector_Shape_W[11] in red
'	WHEN IRD_East[11]=VACANT DO
'		$Erase Sprite(IR_Detector_Grid_E[11])
'	WHEN IRD_West[11]=VACANT DO
		$Erase Sprite(IR_Detector_Grid_W[11])
