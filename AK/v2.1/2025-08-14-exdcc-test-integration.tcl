...
VARIABLES:
    ...
    ' PANEL_1_BLOCK_LABELS[NUM_BLOCKS]           ' Text coordinates for block color indication (disabled with name collpasing)
    PANEL_1_BLOCK_TRACKS_1[NUM_BLOCKS]           ' Color for changing block track colors based on cab+occupancy
    PANEL_1_BLOCK_TRACKS_2[NUM_BLOCKS]           ' Color for changing block track colors for blocks with a crossing
    PANEL_1_BLOCK_DIRECTIONS_1[NUM_BLOCKS]       ' Coordinates to put left/right indicator for block direction (#1, left)
    PANEL_1_BLOCK_DIRECTIONS_2[NUM_BLOCKS]       ' Coordinates to put left/right indicator for block direction (#2, right)
    PANEL_1_BLOCK_SIGNAL_INDICATORS[NUM_BLOCKS]  ' Coordinates for block-occupancy indicator signal (still in progress)
...
SUB Redraw_Turnout(TurnoutIndex)
    ...
    $SWITCH(PANEL_1_TURNOUTS[TurnoutIndex]) = TURNOUT_STATUSES[TurnoutIndex]
ENDSUB
...
SUB Redraw_Debugging(BlockIndex, {local} BlockStatus, {local} CabColor, {local} CabStatusEast, {local} CabStatusWest)
    {--
     - Cab Assignments
     -}
    CabColor = BLOCK_CAB_ASSIGNMENTS[BlockIndex]
    CabColor = CAB_COLORS[CabColor]

    CabStatusEast = BLOCK_STATUSES[BlockIndex]
    CabStatusEast = BLOCK_OCCUPIED_EAST &
    IF CabStatusEast THEN $Draw SPRITE (BLOCK_DEBUGGING_CAB_STATUS[BlockIndex]) = TRAIN_EAST IN CabColor ENDIF

    CabStatusWest = BLOCK_STATUSES[BlockIndex]
    CabStatusWest = BLOCK_OCCUPIED_WEST &
    IF CabStatusWest THEN $Draw SPRITE (BLOCK_DEBUGGING_CAB_STATUS[BlockIndex]) = TRAIN_WEST IN CabColor ENDIF

    IF CabStatusEast = 0, CabStatusWest = 0 THEN $Draw SPRITE (BLOCK_DEBUGGING_CAB_STATUS[BlockIndex]) = CIRCLE IN CabColor ENDIF

    {--
     - Block Occupancy Assignments
     -}
    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_OCCUPIED &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xYxxxx" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "x*xxxx" ENDIF

    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_OCCUPIED_EAST &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "Gxxxxx" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "*xxxxx" ENDIF

    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_OCCUPIED_WEST &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxGxxx" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xx*xxx" ENDIF

    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_UNDER_MANUAL_HOLD &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxxRxx" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxx*xx" ENDIF

    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_UNDER_SYSTEM_HOLD &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxxxRx" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxxx*x" ENDIF

    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_UNDER_MAINLINE_HOLD &
    IF BlockStatus THEN $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxxxxR" ELSE $Signal (BLOCK_DEBUGGING_BLOCK_STATUS[BlockIndex]) = "xxxxx*" ENDIF
ENDSUB
...
SUB Redraw_Block_Occupancy(BlockIndex, {local} BitMaskResult, {local} TrackColor, {local} SignalState, {local} UnderManualHold, {local} UnderMainlineHold)
    ...
    $COLOR BLOCK (PANEL_1_BLOCK_TRACKS_1[BlockIndex]) = TrackColor
    $COLOR BLOCK (PANEL_1_BLOCK_TRACKS_2[BlockIndex]) = TrackColor

    '''
    
    $ERASE SPRITE (PANEL_1_BLOCK_DIRECTIONS_1[BlockIndex])
    $ERASE SPRITE (PANEL_1_BLOCK_DIRECTIONS_2[BlockIndex])

    BitMaskResult = BLOCK_STATUSES[BlockIndex]
    BitMaskResult = BLOCK_OCCUPIED_EAST &
    IF BitMaskResult THEN
        $DRAW SPRITE (PANEL_1_BLOCK_DIRECTIONS_1[BlockIndex]) = ARROW_EAST IN TrackColor
        $DRAW SPRITE (PANEL_1_BLOCK_DIRECTIONS_2[BlockIndex]) = ARROW_EAST IN TrackColor
    ENDIF

    BitMaskResult = BLOCK_STATUSES[BlockIndex]
    BitMaskResult = BLOCK_OCCUPIED_WEST &
    IF BitMaskResult THEN
        $DRAW SPRITE (PANEL_1_BLOCK_DIRECTIONS_1[BlockIndex]) = ARROW_WEST IN TrackColor
        $DRAW SPRITE (PANEL_1_BLOCK_DIRECTIONS_2[BlockIndex]) = ARROW_WEST IN TrackColor
    ENDIF
...
SUB ResetInit_Set_Block_Tracks_On_Panels()
    PANEL_1_BLOCK_TRACKS_1[BLOCK_01] = (37,18,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_02] = (52,18,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_03] = (7,5,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_04] = (22,5,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_05] = (37,5,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_06] = (52,5,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_07] = (7,18,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_08] = (22,18,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_09] = (37,19,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_10] = (7,6,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_11] = (7,19,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_12] = (13,20,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_13] = (6,10,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_14] = (52,9,1)
    PANEL_1_BLOCK_TRACKS_1[BLOCK_15] = (52,22,1)

    PANEL_1_BLOCK_TRACKS_2[BLOCK_01] = (37,18,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_02] = (52,18,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_03] = (7,5,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_04] = (22,5,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_05] = (37,5,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_06] = (52,5,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_07] = (7,18,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_08] = (22,18,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_09] = (37,19,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_10] = (7,6,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_11] = (7,19,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_12] = (13,20,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_13] = (13,14,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_14] = (52,9,1)
    PANEL_1_BLOCK_TRACKS_2[BLOCK_15] = (57,23,1)
ENDSUB

SUB ResetInit_Set_Block_Directions_On_Panels()
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_01] = (37,18,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_02] = (52,18,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_03] = (7,5,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_04] = (22,5,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_05] = (37,5,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_06] = (52,5,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_07] = (7,18,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_08] = (22,18,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_09] = (37,19,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_10] = (7,6,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_11] = (7,19,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_12] = (13,20,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_13] = (6,10,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_14] = (52,9,1)
    PANEL_1_BLOCK_DIRECTIONS_1[BLOCK_15] = (52,22,1)

    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_01] = (43,18,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_02] = (58,18,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_03] = (13,5,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_04] = (28,5,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_05] = (43,5,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_06] = (58,5,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_07] = (13,18,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_08] = (28,18,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_09] = (43,19,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_10] = (13,6,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_11] = (13,19,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_12] = (13,20,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_13] = (6,10,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_14] = (52,9,1)
    PANEL_1_BLOCK_DIRECTIONS_2[BLOCK_15] = (52,22,1)
ENDSUB

SUB ResetInit_Set_Block_Debugging_Mappings()
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_01] = (7, 31, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_02] = (7, 32, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_03] = (7, 33, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_04] = (7, 34, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_05] = (22, 31, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_06] = (22, 32, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_07] = (22, 33, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_08] = (22, 34, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_09] = (37, 31, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_10] = (37, 32, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_11] = (37, 33, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_12] = (37, 34, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_13] = (52, 31, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_14] = (52, 32, 1)
    BLOCK_DEBUGGING_CAB_STATUS[BLOCK_15] = (52, 33, 1)

    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_01] = (8, 31, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_02] = (8, 32, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_03] = (8, 33, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_04] = (8, 34, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_05] = (23, 31, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_06] = (23, 32, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_07] = (23, 33, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_08] = (23, 34, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_09] = (38, 31, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_10] = (38, 32, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_11] = (38, 33, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_12] = (38, 34, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_13] = (53, 31, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_14] = (53, 32, 1)
    BLOCK_DEBUGGING_BLOCK_STATUS[BLOCK_15] = (53, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_01] = (9, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_02] = (9, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_03] = (9, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_04] = (9, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_05] = (24, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_06] = (24, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_07] = (24, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_08] = (24, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_09] = (39, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_10] = (39, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_11] = (39, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_12] = (39, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_13] = (54, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_14] = (54, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_15] = (54, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_01] = (10, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_02] = (10, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_03] = (10, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_04] = (10, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_05] = (25, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_06] = (25, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_07] = (25, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_08] = (25, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_09] = (40, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_10] = (40, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_11] = (40, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_12] = (40, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_13] = (55, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_14] = (55, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_15] = (55, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_01] = (11, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_02] = (11, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_03] = (11, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_04] = (11, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_05] = (26, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_06] = (26, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_07] = (26, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_08] = (26, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_09] = (41, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_10] = (41, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_11] = (41, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_12] = (41, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_13] = (56, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_14] = (56, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_15] = (56, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_01] = (12, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_02] = (12, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_03] = (12, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_04] = (12, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_05] = (27, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_06] = (27, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_07] = (27, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_08] = (27, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_09] = (42, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_10] = (42, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_11] = (42, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_12] = (42, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_13] = (57, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_14] = (57, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_15] = (57, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_01] = (13, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_02] = (13, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_03] = (13, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_04] = (13, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_05] = (28, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_06] = (28, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_07] = (28, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_08] = (28, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_09] = (43, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_10] = (43, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_11] = (43, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_12] = (43, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_13] = (58, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_14] = (58, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_15] = (58, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_01] = (14, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_02] = (14, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_03] = (14, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_04] = (14, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_05] = (29, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_06] = (29, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_07] = (29, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_08] = (29, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_09] = (44, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_10] = (44, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_11] = (44, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_12] = (44, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_13] = (59, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_14] = (59, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_15] = (59, 33, 1)

    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_01] = (15, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_02] = (15, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_03] = (15, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_04] = (15, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_05] = (30, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_06] = (30, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_07] = (30, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_08] = (30, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_09] = (45, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_10] = (45, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_11] = (45, 33, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_12] = (45, 34, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_13] = (60, 31, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_14] = (60, 32, 1)
    BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_15] = (60, 33, 1)
ENDSUB
...
SUB Redraw_All_Blocks({local} BlockIndex)
    BlockIndex = 0
    UNTIL BlockIndex = NUM_BLOCKS LOOP
        Redraw_Block_Occupancy(BlockIndex),
        BlockIndex = 1 +
    ENDLOOP
ENDSUB
...
{-- Mainline Automation --}
SUB Enable_Mainline_Automation()
    MAINLINE_AUTOMATION = ON

    $COLOR TRACK (3, 31, 1) = GREEN
    $SWITCH (3, 31, 1) = OFF
ENDSUB

SUB Disable_Mainline_Automation()
    MAINLINE_AUTOMATION = OFF

    $COLOR TRACK (3, 31, 1) = RED
    $SWITCH (3, 31, 1) = ON
ENDSUB

SUB Toggle_Mainline_Automation()
    IF MAINLINE_AUTOMATION = ON THEN Disable_Mainline_Automation() ELSE Enable_Mainline_Automation() ENDIF
ENDSUB

SUB ResetInit_Mainline_Automation()
    Disable_Mainline_Automation()
ENDSUB
...
SUB Mainline_Hold_Removed(BlockIndex, {local} CurrentMainlineHold)
    CurrentMainlineHold = BLOCK_STATUSES[BlockIndex]
    CurrentMainlineHold = BLOCK_UNDER_MAINLINE_HOLD &

    IF CurrentMainlineHold = 0 THEN RETURN ENDIF

    BLOCK_STATUSES[BlockIndex] = BLOCK_UNDER_MAINLINE_HOLD ^
    Block_Status_Changed(BlockIndex)
ENDSUB

SUB Toggle_Mainline_Hold(BlockIndex, {local} BlockStatus)
    BlockStatus = BLOCK_STATUSES[BlockIndex]
    BlockStatus = BLOCK_UNDER_MAINLINE_HOLD &
    IF BlockStatus THEN Mainline_Hold_Removed(BlockIndex) ELSE Mainline_Hold_Invoked(BlockIndex) ENDIF
ENDSUB
...
{--
 - Mainline Automation
 -}
    WHEN $COMMAND = "Toggle Mainline Automation" DO Toggle_Mainline_Automation()
    WHEN $LeftMouse = (3, 31, 1) DO Toggle_Mainline_Automation()
...
{--
 ...
 -}
    ...
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_05] DO Block_Stopped_Triggering(Block_05)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_06] DO Block_Stopped_Triggering(Block_06)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_07] DO Block_Stopped_Triggering(Block_07)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_08] DO Block_Stopped_Triggering(Block_08)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_09] DO Block_Stopped_Triggering(Block_09)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_10] DO Block_Stopped_Triggering(Block_10)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_11] DO Block_Stopped_Triggering(Block_11)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_12] DO Block_Stopped_Triggering(Block_12)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_13] DO Block_Stopped_Triggering(Block_13)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_14] DO Block_Stopped_Triggering(Block_14)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_UNOCCUPIED[BLOCK_15] DO Block_Stopped_Triggering(Block_15)
    ...
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_05]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_06]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_07]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_08]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_09]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_10]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_11
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_12]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_13]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_14]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_OCCUPIED[BLOCK_15]
    ...
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_05] DO Block_Triggered_East(Block_05)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_06] DO Block_Triggered_East(Block_06)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_07] DO Block_Triggered_East(Block_07)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_08] DO Block_Triggered_East(Block_08)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_09] DO Block_Triggered_East(Block_09)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_10] DO Block_Triggered_East(Block_10)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_11] DO Block_Triggered_East(Block_11)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_12] DO Block_Triggered_East(Block_12)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_13] DO Block_Triggered_East(Block_13)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_14] DO Block_Triggered_East(Block_14)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_EAST[BLOCK_15] DO Block_Triggered_East(Block_15)
    ...
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_05] DO Block_Triggered_West(Block_05)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_06] DO Block_Triggered_West(Block_06)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_07] DO Block_Triggered_West(Block_07)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_08] DO Block_Triggered_West(Block_08)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_09] DO Block_Triggered_West(Block_09)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_10] DO Block_Triggered_West(Block_10)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_11] DO Block_Triggered_West(Block_11)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_12] DO Block_Triggered_West(Block_12)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_13] DO Block_Triggered_West(Block_13)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_14] DO Block_Triggered_West(Block_14)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_WEST[BLOCK_15] DO Block_Triggered_West(Block_15)

    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_01] DO Toggle_Manual_Hold(Block_01)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_02] DO Toggle_Manual_Hold(Block_02)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_03] DO Toggle_Manual_Hold(Block_03)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_04] DO Toggle_Manual_Hold(Block_04)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_05] DO Toggle_Manual_Hold(Block_05)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_06] DO Toggle_Manual_Hold(Block_06)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_07] DO Toggle_Manual_Hold(Block_07)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_08] DO Toggle_Manual_Hold(Block_08)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_09] DO Toggle_Manual_Hold(Block_09)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_10] DO Toggle_Manual_Hold(Block_10)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_11] DO Toggle_Manual_Hold(Block_11)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_12] DO Toggle_Manual_Hold(Block_12)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_13] DO Toggle_Manual_Hold(Block_13)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_14] DO Toggle_Manual_Hold(Block_14)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MANUAL_HOLD[BLOCK_15] DO Toggle_Manual_Hold(Block_15)

    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_01]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_02]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_03]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_04]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_05]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_06]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_07]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_08]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_09]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_10]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_11]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_12]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_13]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_14]
    ' WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_SYSTEM_HOLD[BLOCK_15]

    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_01] DO Toggle_Mainline_Hold(Block_01)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_02] DO Toggle_Mainline_Hold(Block_02)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_03] DO Toggle_Mainline_Hold(Block_03)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_04] DO Toggle_Mainline_Hold(Block_04)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_05] DO Toggle_Mainline_Hold(Block_05)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_06] DO Toggle_Mainline_Hold(Block_06)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_07] DO Toggle_Mainline_Hold(Block_07)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_08] DO Toggle_Mainline_Hold(Block_08)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_09] DO Toggle_Mainline_Hold(Block_09)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_10] DO Toggle_Mainline_Hold(Block_10)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_11] DO Toggle_Mainline_Hold(Block_11)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_12] DO Toggle_Mainline_Hold(Block_12)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_13] DO Toggle_Mainline_Hold(Block_13)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_14] DO Toggle_Mainline_Hold(Block_14)
    WHEN $LeftMouse = BLOCK_DEBUGGING_BLOCK_TOGGLE_MAINLINE_HOLD[BLOCK_15] DO Toggle_Mainline_Hold(Block_15)

SUB ExDcc_ThrowTurnout({local} TurnoutIndex)
    TurnoutIndex = $IOData
    TurnoutIndex = 1 -

    Set_Turnout(TurnoutIndex, ON)
ENDSUB

SUB ExDcc_CloseTurnout({local} TurnoutIndex)
    TurnoutIndex = $IOData
    TurnoutIndex = 1 -

    Set_Turnout(TurnoutIndex, OFF)
ENDSUB

WHEN $InBufLen >= 1 DO
    DccEx_Command = $IOData

    $Log = "DCC-EX Command Received: @DccEx_Command"

    IF DccEx_Command = 100 THEN ExDcc_ThrowTurnout() ENDIF
    IF DccEx_Command = 101 THEN ExDcc_CloseTurnout() ENDIF

WHEN $LeftMouse = (23, 24, 1) DO
    $Log = "Button Hit ->",
    $IOData = 300,
    $IOData = 16,
